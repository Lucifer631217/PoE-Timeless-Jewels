...
const obj = {
  ...
  async search(args: ReverseSearchConfig, callback: (seed: number) => Promise<void>): Promise<SearchResults> {
    const searchResult =
      (await calculator.ReverseSearch(
        args.nodes,
    ...
    Object.keys(searchResult).forEach((seedStr) => {
      ...
      const skills = Object.keys(seedResult).map((skillIDStr) => {
        const skillID = parseInt(skillIDStr);
        const skillStats = seedResult[skillID] || {};
      ...
      const len = Object.keys(seedResult).length;
      searchGrouped[len] = [
        ...(searchGrouped[len] || []),
        {
          skills: skills,
    ...
    return {
      ...
      raw: Object.values(searchGrouped)
        .flat()
