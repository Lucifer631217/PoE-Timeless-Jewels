...
func (ds *dataState) readRow(id int) (any, error) {
	...
	for i, field := range ds.rowFormat.Fields {
		...
		if err != nil {
			return nil, fmt.Errorf("error reading field [%d] %s of row %d: %w", i, field.Name, id, err)
		}
	}
...
func (ds *dataState) readField(tgt reflect.Value, typ FieldType, rowdat []byte, dyndat []byte) error {
	...
	case TypeBool:
		if len(rowdat) < 1 {
			return fmt.Errorf("row too short for bool field %s in row %d", ds.curField, ds.curRow)
		}
		val := rowdat[0]
